import { LogOut, X, Moon, Sun, type LucideIcon } from "lucide-react";
import { useTheme } from "../contexts/ThemeContext";

interface SidebarProps {
    activeTab: string;
    setActiveTab: (tab: string) => void; // ✅ แก้ไข any -> string
    menuItems: { id: string; label: string; icon: LucideIcon }[]; // ✅ แก้ไข any -> LucideIcon
    userRole: string;
    username: string;
    onLogout: () => void;
    isOpen: boolean;
            ...
            >
                ...
                <div className="p-6 pb-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[15px] text-black dark:text-white font-bold uppercase tracking-widest mt-0.5">
                                Shop9Bath
                            </p>
                    ...
                    <button
                        onClick={onClose}
                        className="md:hidden p-2 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800"
                    >
                        <X size={24} />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 space-y-1">
                    <p className="px-4 py-2 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">
                        Main Menu
                    </p>
                    {menuItems.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-all duration-200 group relative ${
                                    isActive
                                        ? "bg-indigo-50 text-indigo-700 dark:bg-zinc-800 dark:text-white font-bold shadow-sm"
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <Icon
                                        size={20}
                                        className={`transition-colors ${isActive ? "text-indigo-600 dark:text-white" : "text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300"}`}
                                    />
                ...
                <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 space-y-3">
                    ...
                    >
                        ...
                        <div
                            className={`w-8 h-4 rounded-full relative transition-colors ${theme === "dark" ? "bg-indigo-500" : "bg-zinc-300"}`}
                        >
                            <div
                                className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${theme === "dark" ? "left-4.5" : "left-0.5"}`}
                            />
                        </div>
                    </button>
                    ...
                    <div className="bg-zinc-50 dark:bg-zinc-900 rounded-2xl p-4 border border-zinc-200 dark:border-zinc-800 group hover:border-indigo-100 dark:hover:border-zinc-700 transition-colors">
                        <div className="flex items-center gap-3 mb-3">
                            <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-md min-w-10 ${
                                    userRole === "director"
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-zinc-400 dark:text-zinc-500 capitalize truncate">
                                    {userRole}
                                </p>
                            </div>
                        </div>
