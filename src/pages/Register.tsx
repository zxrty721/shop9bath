...
export default function Register() {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // ✅ 2. แปลง (Cast) err เป็น AxiosError
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-zinc-100 dark:bg-linear-to-br dark:from-zinc-900 dark:via-black dark:to-zinc-900 font-prompt p-4 relative overflow-hidden transition-colors duration-500">
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                ...
                <div
                    className="absolute bottom-[-10%] left-[-5%] w-96 h-96 bg-blue-600/20 rounded-full animate-pulse"
            ...
            <div className="bg-white dark:bg-zinc-900/90 backdrop-blur-xl p-8 md:p-10 rounded-3xl shadow-2xl w-full max-w-md z-10 border border-zinc-200 dark:border-zinc-800 animate-fade-in transition-colors duration-300">
                <div className="text-center mb-8">
                    ...
                    <p className="text-zinc-500 dark:text-zinc-400 text-xs mt-2 uppercase tracking-widest font-semibold">
                        SHOP9BATH REGISTRATION
                    </p>
                </div>
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                    <div className="relative group">
                        <BadgeCheck
                            className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors z-10"
                            size={20}
                        />
                        <input
                            type="text"
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className={`w-full bg-zinc-900 hover:bg-black dark:bg-white dark:hover:bg-zinc-200 text-white dark:text-black font-bold py-3.5 rounded-xl shadow-lg hover:shadow-xl transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 mt-6 ${isLoading ? "opacity-70 cursor-not-allowed" : ""}`}
                    >
                ...
                <div className="mt-8 text-center border-t border-zinc-100 dark:border-zinc-800 pt-6">
                    <Link
                        to="/"
                        className="inline-flex items-center text-sm font-bold text-zinc-600 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors group"
                    >
                        <ArrowLeft
                            size={16}
